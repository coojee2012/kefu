...
export class CCQueue {
    ...
    constructor(private injector: Injector) {
        ...
        this.addInQueueJobTimes = 0;
        this.queueAnswered = false;
   ...
   */
    async dialQueue(queueNumber: string): Promise<dialQueueResult> {
        try {
            ...
            if (this.queue && this.cgrReqType) {
                ...
                this.fsPbx.addConnLisenter(callerHangupEvent, 'once', this.onCallerHangupHandle.bind(this))
                        ...
                        const onDisconnect = async (evt) => {
                            try {
                                ...
                                if (evt.getHeader('Controlled-Session-UUID') === callId) {
                                    if (!doneOver) {
                                        this.logger.debug(`FIND_QUEUE_MEMBER_DONE ON DISCONNECT!`);
                                        doneOver = true;
    ...
    addBullQueueMonitor() {
        ...
        this.bullQueue.on('completed', this.onFindQueueMember.bind(this));
    ...
    async startFindMemberJob() {
        try {
            const { tenantId, callId, caller, callee: called, routerLine } = this.runtimeData.getRunData();
            this.logger.debug(`Start Find Member!Pass ${this.addInQueueJobTimes} Times!`);
            const { queueName, queueNumber, queue: queueConf } = this.queue;
            const { maxWaitTime, enterTipFile } = queueConf;
            ...
            this.logger.debug('A Find Queue Member To Job:', this.queueJob ? this.queueJob.id : 'NULL');
    ...
    async onCallerHangupHandle(evt) {
        try {
    ...
    async callerHangupOnWaiting() {
        ...
        try {
            ...
            if (this.originationUuid) {
                tasks.push(
                    this.fsPbx.uuidTryKill(this.originationUuid, `Caller Is Hangup Yet!${this.originationUuid}`)
                )
            }
    ...
    async dialQueueMember({ agentInfo }): Promise<dialQueueMemberResult> {
        ...
        try {
            ...
            this.logger.debug('dial a  member dialStr:', dialStr);
            ...
            if (oriResult && oriResult.success) {
                ...
                this.logger.debug('bridge agent and user:', dialResult);
    ...
    async dialQueueTimeOut() {
        try {
            ...
            while (reReadDigits && !agentAnswered) {
                reReadDigits = false;
                // 提示是否继续等待音
                this.logger.info('Tip Is Need To Continue Wait!');
                const inputKey = await this.fsPbx.uuidRead(readArgs);
                if (inputKey === '1') {
                    this.logger.info('User Choice Wait Again!');
                    result.wait = true;// 再排队一次
                }
                else if (inputKey === 'timeout') {
                    // 输出超时音
                    this.logger.info('Tip When Wait Input A key Timeout!');
                    await this.fsPbx.uuidPlayback({
                        uuid: callId,
                        terminators: 'none',
                    ...
                    // 输入错误音
                    this.logger.debug('Tip User Input A Error Key!');
