...
export class Parser extends EventEmitter2 {
  ...
  _parseHeaders() {
    ...
    //otherwise, this even is completed create an esl.Event object from it
      ...
      this._parseEvent(this.headers,null);
  ...
  _parseHeaderText(txt) {
    if(!txt){
      console.log('_parseHeaderText','wwwwwwwwwwwwwwwwwwww')
      return ;
    }
    return txt.split('\n').reduce(function (headers, line) {
      ...
      if (key == 'variable_sip_invite_record_route' || key == 'variable_sip_recover_via') {
        //console.log('跳过:variable_sip_invite_record_route或variable_sip_recover_via')
      } else {
        try {
          value = decodeURIComponent(data.join(': '));
        } catch (ex) {
          console.error('decodeURIComponent error:', key,data);
      ...
      if (key === 'Content-Length') {
        try {
          ...
          console.error('parseInt error:', key, value);
          console.error('parseInt error:', ex);
        }
      } else {
        headers[key] = value;
      }
  ...
  _parseXmlBody(txt) {
    ...
    //  <body>...</body> [optional]
    //</event>
    let parser = new xml2js.Parser({explicitArray: false, explicitRoot: false}),
        self = this,
        headers = {};
