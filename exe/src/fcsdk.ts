...
const URL_HUOBI_PRO = 'api.fcoin.com';
const BASE_URL = 'https://api.fcoin.com/v2';
...
export class FCSDK {
    ...
    sign_sha(method:string, baseurl, path, data) {
        this.privateKey = '3600d0a74aa3410fb3b1996cca2419c8';
        var pars = [];
        ...
        var ts = 1523069544359 || moment.utc().format('YYYY-MM-DDTHH:mm:ss');
        var meta = [method, baseurl+path,ts, p].join('');
        this.logger.debug(meta);
        meta  = Buffer.from(meta).toString('base64')
        this.logger.debug(meta);
        var hash = HmacSHA1(meta, this.privateKey);
    
        this.logger.debug('hash:',hash);
        var Signature = CryptoJS.enc.Base64.stringify(hash); //encodeURIComponent(CryptoJS.enc.Base64.stringify(hash));
